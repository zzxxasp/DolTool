...
public class HoloCircularProgressBar extends View {
	...
	protected void onDraw(final Canvas canvas) {
		...
		// draw the background
		if (!mOverrdraw) {
			canvas.drawArc(mCircleBounds, 125, 290, false, mBackgroundColorPaint);
		}

		
		// draw the progress or a full circle if overdraw is true
		canvas.drawArc(mCircleBounds, 125, mOverrdraw ? 290 : progressRotation, false, mProgressColorPaint);
		canvas.drawArc(mCircleBounds2,0,290, false, mBackgroundColorPaint);
		...
		if (isThumbEnabled()) {
			...
			canvas.save();
			canvas.rotate(progressRotation - 235);
			canvas.drawCircle(mThumbPosX, mThumbPosY,DensityUtil.dip2px(getContext(),3.3f), mThumbColorPaint);
			canvas.restore();
		}
	}
	...
	protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
		...
		mThumbPosX = (float) ((mRadius-DensityUtil.dip2px(getContext(),23.3f)) * Math.cos(0));
		mThumbPosY = (float) ((mRadius-DensityUtil.dip2px(getContext(),23.3f))  * Math.sin(0));
...
//		//0xfff87d7a
//		gradient = new SweepGradient(getWidth()/2,getWidth()/2,0xffe13e4d,0xfff87d7a);
//		Matrix matrix = new Matrix();
//		matrix.setRotate(125,getWidth()/2,getWidth()/2);
